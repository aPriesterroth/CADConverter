...
import cad.libraries.effects.EffectsLibrary;
...
import cad.libraries.materials.MaterialsLibrary;
...
public class CADRawObject {

    /**
     * Idea to take everything from CADObject2 and move it to here and make CADObject2 the clean version of a model
     * where later all you have to do is extend your class CADObject2, make a constructor with filepath and pass it
     ...
     *
     * CADObject2 is a "trimmed" version of the CADRawObject.
     *      -> CADRawObject contains everything (libraries, data, joints, filepath, etc.)
     *      -> CADObject2 contains only required information (data, joints, etc.)
     */

    private static final CADMatrix4f BLENDER_CORRECTION = new CADMatrix4f().rotate((float) Math.toRadians(-90),
            new CADVector3f(1, 0, 0));
    ...
    private ArrayList<CADVertexSkinData> skinningData;

    private int jointCount;
    private CADJoint rootJoint;

    private ArrayList<CADVertex> verticesList;
    private ArrayList<CADVector2f> texturesList;
    private ArrayList<CADVector3f> normalsList;
    ...
    /**
     * Creates a CADRawObject based on a filepath to a ".dae" file, initializes libraries and the parsing of data from
     ...
     */
    public CADRawObject(String filepath, boolean correctBlenderCoordinates) {
    ...
    private void initializeLibraries() {
        CADParser parser = new CADParser(filepath);
    ...
    /**
     * Parsing the joint order and "CADVertexSkinData" skinning data from a specific ControllersLibrary.
     *
    ...
    private void parseDataFromControllersLibrary(ControllersLibrary library) {
        ...
        ArrayList<CADVertexSkinData> skinningData = new ArrayList<>();
        ...
        for(int count : library.getVertexWeights().getVCount()) {
            CADVertexSkinData skinData = new CADVertexSkinData(3);
    ...
    /**
     * Parsing the "CADJoint" rootJoint from a specific VisualScenesLibrary.
     *
        ...
        for(Node parent : library.getNodes()) {
            for(Node child : parent.getChildren()) {
                ...
                if(child.getName().equals(rootJointName)) {
                    ...
                    CADMatrix4f localBindMatrix = new CADMatrix4f(child.getMatrix()).transpose();
                    ...
                    if(child.getType().equals("JOINT")) {
                        rootJoint = new CADJoint(id, name, child.getSid(), localBindMatrix, null);
                    } else {
                        rootJoint = new CADJoint(id, name, localBindMatrix, null);
                    }
    ...
    /**
     * Parses an array of "CADJoint" children from a specific node based on a specific "CADJoint" parent and returns it.
     *
     * @param parent - the parent specified for the parsed children
     * @param node - the specific node to parse the children from
     *
     * @return - the array of "CADJoint" children
     */
    private CADJoint[] parseJointChildren(CADJoint parent, Node node) {
        ...
        for(int i = 0; i < node.getChildren().length; i++) {
            ...
            if(child.getType().equals("JOINT")) {
                children[i] = new CADJoint(id, name, child.getSid(), localBindMatrix, parent);
            } else {
                // Used to be:
                // new CADJoint(node.getId(), node.getName(), new CADMatrix4f(node.getMatrix()).transpose(), parent, null);
                children[i] = new CADJoint(id, name, localBindMatrix, parent);
            }
    ...
    private void parseVerticesList(float[] positions) {
        for(int i = 0; i < positions.length / 3; i++) {
            ...
            CADVector4f posVector = new CADVector4f(x, y, z, 1);

            if(correctBlenderCoordinates()) {
                posVector = posVector.transform(BLENDER_CORRECTION);
            }

            verticesList.add(new CADVertex(verticesList.size(), new CADVector3f(posVector.x, posVector.y, posVector.z),
    ...
    private void parseTexturesList(float[] textures) {
        for(int i = 0; i < textures.length / 2; i++) {
            ...
            texturesList.add(new CADVector2f(s, t));
        }
    }
    ...
    private void parseNormalsList(float[] normals) {
        for(int i = 0; i < normals.length / 3; i++) {
            ...
            CADVector4f normal = new CADVector4f(x, y, z, 0.0f).transform(BLENDER_CORRECTION);

            normalsList.add(new CADVector3f(normal.x, normal.y, normal.z));
        }
    }
    ...
    /**
     * Sets the textureIndex and normalIndex of a specific "CADVertex" vertex specified by a specific positionIndex
     * inside the ArrayList verticesList.
     * The vertex is passed to {@link #setVertexIndicesDuplicate(CADVertex, int, int)} if a
    ...
    private void setVertexIndices(int positionIndex, int textureIndex, int normalIndex) {
        CADVertex vertex = verticesList.get(positionIndex);
     ...
     */
    private void setVertexIndicesDuplicate(CADVertex vertex, int newTextureIndex, int newNormalIndex) {
        if(vertex.indicesEquals(newTextureIndex, newNormalIndex)) {
            indicesList.add(vertex.getIndex());
        } else {
            CADVertex duplicate = vertex.getDuplicate();

            if(duplicate == null) {

                duplicate = new CADVertex(verticesList.size(), vertex.getPosition(), vertex.getWeightsData());
    ...
    /**
     * Sets the texture and normal indices of every "CADVertex" vertex contained the ArrayList verticesList, if the
     * vertex indices have not been set yet.
     */
    private void trimVerticesList() {
        for(CADVertex vertex : verticesList) {
            if(!vertex.hasIndiciesSet()) {
    ...
    private void initializeDataArrays() {
        ...
        for(int i = 0; i < verticesList.size(); i++) {
            CADVertex vertex = verticesList.get(i);

            CADVector3f position = vertex.getPosition();
            CADVector2f texture = texturesList.get(vertex.getTextureIndex());
            CADVector3f normal = normalsList.get(vertex.getNormalIndex());
            ...
            CADVertexSkinData skinData = vertex.getWeightsData();
