...
/**
 * Implementation of an CADObject2 consisting of basic geometry data (vertices, normals, textureCoords and all
...
public class CADObject2 {
    ...
    public CADObject2(String path){
        ...
        try {
            ...
            //rootJoint.calculateInverseBindTransform(new CADMatrix4f());

        } catch (ParsingException e){
            e.printStackTrace();
        }

        System.out.println("Finished loading the CADObject2 (" + this + ") from path: " + path + "");
    }
     ...
     * GeometriesLibrary to the CADObject2 instance.
    ...
    /**
     * Parses the texture file name from a specific ImagesLibrary to the CADObject2 instance.
     *
            ...
            for(Node node : library.getVisualScene().getNodes())
                for(Node node2 : node.getChildren())
                    if(node2.getName().equals(rootName)){
                        ...
                        CADMatrix4f matrix = new CADMatrix4f(node2.getMatrix()).transpose();
                        ...
                        if(node2.getType().equals("JOINT")){
                            root = new CADJoint(node2.getId(), node2.getName(), node2.getSid(), matrix,
                                    null);
                            root.setChildren(parseJointChildren(root, node2));

                        } else {
                            root = new CADJoint(node2.getId(), node2.getName(), matrix,
                                    null);
                            root.setChildren(parseJointChildren(root, node2));
                        }
        ...
        for(int i = 0; i < node.getChildren().length; i++){
            ...
            if(node.getChildren()[i].getType().equals("JOINT")){
                ...
                children[i] = new CADJoint(node.getChildren()[i].getId(), node.getChildren()[i].getName(), node.getChildren()[i].getSid(),
                        new CADMatrix4f(node.getChildren()[i].getMatrix()).transpose(), parent);
            } else {

                children[i] = new CADJoint(node.getId(), node.getName(), new CADMatrix4f(node.getMatrix()).transpose(),
                        parent);
            }
     ...
     * Controllers library to the CADObject2 instance.
     *
        ...
        for(Source source : library.getController().getSkin().getSources()){
            if(source.getId().equals(jointsSourceStr)){
                ...
                for(int i = 0; i < names.length; i++){
                    ...
                    curJoint.setInverseBindTransform(new CADMatrix4f(components));
                    curJoint.setIndex(i);
                }
            }
                ...
                for(Animation a : library.getAnimations()) {
                    ...
                    for(Source s : a.getSources())
                        if(s.getId().contains("output")) {
                            ...
                            CADMatrix4f matrix = new CADMatrix4f(components);

                            CADVector3f position = new CADVector3f(matrix.m30, matrix.m31, matrix.m32);
                            CADQuaternion rotation = new CADQuaternion(matrix);
     ...
     * @return -
     */
    public CADMatrix4f[] getJointTransforms() {
        CADMatrix4f[] jointMatrices = new CADMatrix4f[jointCount];
     ...
     * @param jointMatrices -
     */
    private void addJointsToArray(CADJoint root, CADMatrix4f[] jointMatrices) {
